...
const lint = fileURLToPath(new URL("../scripts/memory-lint.mjs", import.meta.url));
...
describe("memory consolidation", () => {
  test("is byte-identical, provenance-pointed, and leaves episodic sources untouched", async () => {
    ...
    await Promise.all([
      writeFile(join(root, "fort", "memory", "facts", "current-truth.md"), fact),
      writeFile(join(root, ".beads", "issues.jsonl"), '{"id":"x","status":"open","title":"Open work","updated_at":"2026-08-10T00:00:00Z"}\n'),
      writeFile(join(root, "fort", "handoffs", "forge-2026-08-10.md"), "# Handoff: Forge 2026-08-10T01:00:00Z\n\n## State of work\n\nReady.\n\n## Next actions\n\nShip it.\n"),
      writeFile(join(root, "fort", "events", "events-2026-08-10.jsonl"), '{"ts":"2026-08-10T02:00:00Z","category":"incident","detail":"Needs attention"}\n'),
    ]);
    ...
    const first = await readFile(join(root, "fort", "memory", "current.md"), "utf8");
    await run(process.execPath, [assembler, root]);
    expect(await readFile(join(root, "fort", "memory", "current.md"), "utf8")).toBe(first);
