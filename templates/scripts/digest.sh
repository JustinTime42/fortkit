...
all_events="$(mktemp)"; timestamped_events="$(mktemp)"; window_events="$(mktemp)"; closed_beads="$(mktemp)"; timestamp_report="$(mktemp)"; audit_merges="$(mktemp)"; audit_coverage="$(mktemp)"; live_human_beads="$(mktemp)"; all_beads="$(mktemp)"; action_render="$(mktemp)"; subject_render="$(mktemp)"
trap 'rm -f "$all_events" "$timestamped_events" "$window_events" "$closed_beads" "$timestamp_report" "$audit_merges" "$audit_coverage" "$live_human_beads" "$all_beads" "$action_render" "$subject_render"' EXIT
...
elif [ "$(jq -r 'type' "$live_human_beads")" != "array" ]; then
  printf '  HUMAN QUEUE: UNAVAILABLE (bd returned invalid JSON)\n'
else
  if node - "$live_human_beads" "$max_decisions_per_action" >"$action_render" <<'NODE'
const fs = require("node:fs");
const [beadsPath, limitArg] = process.argv.slice(2);
const beads = JSON.parse(fs.readFileSync(beadsPath, "utf8"));
const limit = Number(limitArg);
const shown = [...beads].sort((left, right) => String(right.updated_at ?? right.created_at ?? "").localeCompare(String(left.updated_at ?? left.created_at ?? ""))).slice(0, limit);
console.log(`  HUMAN QUEUE: ${beads.length} decision(s) waiting (showing ${shown.length} of ${beads.length}, most recently updated)`);
for (const bead of shown) console.log(`    ${bead.title ?? "untitled"} [${bead.id ?? "UNKNOWN"}; ${bead.status ?? "open"}]`);
if (beads.length > shown.length) console.log(`    ${beads.length - shown.length} decision(s) elided; full count is ${beads.length}.`);
NODE
  then
    cat "$action_render"
  else
    printf '  HUMAN QUEUE: UNAVAILABLE (renderer failed)\n'
  fi
fi
...
if [ "$by_subject" -eq 1 ]; then
  ...
  elif [ "$live_human_status" -ne 0 ]; then
    printf '  SUBJECT VIEW: UNAVAILABLE (the human queue is unavailable)\n'
  else
    if node - "$live_human_beads" "$all_beads" >"$subject_render" <<'NODE'
const fs = require("node:fs");
...
for (const { root, members } of [...groups.values()].sort((left, right) => String(left.root.title ?? "").localeCompare(String(right.root.title ?? "")))) {
  ...
  // The filtered human query is intentionally lean. Dependencies must come from
...
const mergeTarget = (subject) => /^Merge ([A-Za-z0-9.-]+):/.exec(subject)?.[1] ?? null;
