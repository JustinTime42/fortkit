...
// The factory that founded this fort reads the registry the same way, FORT_REGISTRY
// first. Honouring the same variable is what lets a test — and a masked seat, for
// whom $HOME/.claude is kernel read-only — point this lint at a registry it can reach.
const registryPath =
...
// that matters is the one inside a single roster, where a one-letter difference
// between two of your own seats' ids mis-attributes events, beads and verdicts.
// Cross-fort near-collisions must NOT be refused: two actor ids in two different
// settlements of this civilization already sit at distance 2 today, legitimately.
...
// provenance, not inheritance — fort/scripts/lib/seat-sandbox.sh as shipped carries
// two such credits — and a whole-file scan would refuse them. The rule is about the
// Held-by and Personality lines and about nothing else.
function parseSeat(text) {
  ...
  if (heldByLine !== null && !heldByLine.includes("{{")) {
    // Filled form: `**Held by: <Given Family>** (pronouns, declared ...)`.
...
// word of the name, lowercased. When an explicit actor-id field exists it lands
// here, and this function is the single place that has to change.
...
// The originating fort had already paid for this once, in a different verifier step:
// a check that resolved paths against the CURRENT tree was false in every checkout
// but one, and it broke the Forge's whole lane because forge.sh runs the verifier
// from a worktree. Resolve the canonical checkout from
...
// THE PRE-MOOT SIGNAL ALREADY EXISTS AND DID NOT NEED INVENTING: the factory writes
// each new fort into the registry with "fort_name": null, and the moot is what
...
// ---------------------------------------------------------------------------
// ONE DIRECTION IS CHECKED, AND THE DECLINED DIRECTION IS PRINTED SO NO READER
// MISTAKES IT FOR COVERAGE.
//
// When fort/seats/*.md and the charter's occupants line disagree, the SEAT FILE is
// authoritative, and every name in one must appear in the charter. The converse —
// every citizen named in the charter must hold a seat — is NOT checked, because a
// charter's occupants text is prose and not a roster: the originating fort's charter
// names three citizens in a single sentence and a fourth in a different paragraph two
// lines later. A lint that parses that reliably does not exist, and one that guesses
// goes red on a benign rewording. fort/memory/memory-lint.mjs records the same
// judgement being made for the same reason after two measured false reds.
// ---------------------------------------------------------------------------
