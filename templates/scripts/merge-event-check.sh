#!/bin/bash
# Audit fence for merges. A configurable lower bound lets a newly founded fort
# begin coverage at its own chosen checkpoint while making every later main
# merge accountable to an append-only merge event.
set -euo pipefail
