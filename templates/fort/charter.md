...
- **The Researcher's WebFetch is an outbound channel, and this residual arrives UNDISCHARGED.** Your Researcher reads the open web, so an injected page can try to induce it to encode repository content into a subsequent fetch URL. Domain-scoping was considered and rejected as crippling to the seat's purpose: a research seat that may read only a whitelist is not a research seat. What bounds the exposure is capability separation — the seat that reads about the world is never the seat that touches it — together with what the seat can reach and what it can do, and the untrusted-input standing order, which makes fetched content data to cite and never instruction to follow. Origin: Manyhalls `fortkit-vhk.5.3`, carried to this fort at founding. **THIS IS THE ONLY RESIDUAL IN THIS SECTION THAT ARRIVES WITHOUT ITS MEASUREMENT, and the heading above is why that matters:** a residual is admitted here only with a measurement attached, and your fort has taken none. The figures that justified accepting this one were measured against Manyhalls' profile and Manyhalls' mask, not yours, and a check that establishes nothing about a differently-configured fort is not evidence about yours. Run `fort/scripts/probe-researcher-boundaries.sh` against your own profile and record your own numbers here before you treat this paragraph as settled. Until you do, this is a gap nobody has measured wearing the clothes of one somebody accepted.
