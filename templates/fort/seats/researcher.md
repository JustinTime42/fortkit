...
**Occupant:** Claude Code, **read-only-toward-the-world by construction**. The launcher grants a tool set of `WebSearch`, `WebFetch`, and read-only local tools (`Read`, `Grep`, `Glob`) and NOTHING else — no `Bash` (a shell defeats any network deny-list), no `Edit`/`Write`, no `Task`/Agent tool. It does NOT run `--dangerously-skip-permissions` and does not inherit `bypassPermissions`. What that buys is precise, and was measured in Manyhalls rather than assumed (`fortkit-p4rc`, 2026-08-10): an unlisted **tool** fails shut — in non-interactive `-p` mode nothing gates at all, because no human is at a prompt, so the tool is simply not there to invoke; an out-of-scope local **read** gates and returns zero bytes; and the two **outbound** tools never gate, being pre-authorized in the profile. The boundary is the absence of the tool, not a checkpoint someone answers. Kernel mask like every seat. Ladder: {{set at seating — a model tier appropriate to research breadth}} → escalate to the dispatching Mayor. Every research output records which model produced it.
