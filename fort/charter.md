...
12. **Architecture ports between forts; identity never does.** WHAT TRAVELS: capabilities, restrictions, sandbox and mask configuration, permission profiles, the memory-store mechanism, launcher mechanics, the protocol sections of a seat file, and the standing orders a seat works under. WHAT STAYS: a seat's name, pronouns and personality, its History and Laurels, and that fort's own memories, handoffs, annals and moot record. That a fort must never inherit another settlement's citizen has been law since the Founding Moot; this extends it from FOUNDING to PORTING, which is where it was never written. **The two mix inside a single file, so the rule is applied per hunk and never per file:** `fort/scripts/mayor.sh` diverged from the template both by its Mayor's name and by a push-gate hardening the template lacked. Port the second, never the first. **That example is written in the past tense because it has since completed, and watching it complete is the clearest statement of the rule this fort has:** the push gate is now in `templates/fort/scripts/mayor.sh:39` and the divergence is gone, while the name half never can be — the template renders "the Mayor of the `{{PROJECT}}` fort" and the three live forts say Emrith Cairnwright, Marrek Splitstone and Vardis Slowfathom. Architecture arrives; citizens stay. (Amended 2026-08-17, `fortkit-hpmn`, Overseer approval on the bead. The clause said "a push-gate hardening the template **lacks**", present tense, after the hardening had been ported — a claim left standing over an edited subject, inside the standing order about porting, and it was found only because `fortkit-6fga` was about to copy it verbatim into two more settlements.) **If you cannot separate them, stop and ask — converging the file is not the safe default, it is the destructive one.** A porting instrument that cannot tell the two apart is not yet fit to run (`fortkit-9qts`). (Overseer amendment 2026-08-13, `fortkit-k1pq`, approval on the bead, adopted before the parity programme rather than after its first casualty.)
