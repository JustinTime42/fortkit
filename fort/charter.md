...
- **`fort/scripts/` accepts NEW FILES from the Mayor's seat** (fortkit-6ovg, accepted 2026-08-12, on the Overseer's ruling of 2026-08-11 to make the `verify.sh` carve-out actually work). Every existing file in that directory is a read-only bind and therefore a MOUNT POINT, so none can be modified, deleted, or renamed over (EBUSY) — but the directory itself is writable in the Mayor's posture, which is exactly what lets `Edit`, `sed -i` and `git checkout` change `verify.sh` at all. A seat can therefore CREATE a new script in a host-executed directory. Nothing there is auto-executed and every launcher is named explicitly, so this is a staging area for a later mistake rather than a direct path. **Held by the MAYOR, the HERALD and the CHRONICLER** — every `build_mask claude` caller that does not pass its own checkout as `extra_ro`: `fort/scripts/mayor.sh:40`, `civ/scripts/herald.sh:97`, `civ/scripts/chronicler.sh:95`. The Warden (`warden.sh:151`) and the Researcher (`researcher.sh:49`) do pass theirs, and the Forge keeps the whole-directory lock, so none of those three hold it. **The Herald is the one that matters most and was missed: it runs headless and unattended on a daily 05:00 timer with `Bash`, `Write` and `Edit`**, so this capability is not confined to attended seats that ask before acting. The code fix is not available — passing `$root` as `extra_ro` to `herald.sh` would make `fort/events/` read-only and the Herald could not emit — so the honest move is to name the real set. Whether the Herald *should* hold it is open (`fortkit-vkq8`). Measurement: E2 harness 52/52 — A4 unlink and rename both refused, A6 new file created and probed as an EXPECTED pass, B2 hole absent in the Warden posture, C4b absent for the Forge — and `fort/scripts/probe-cycle7.sh` 32/32 on the live capital, where the hole is asserted rather than assumed. If that line ever reads FAIL, the mask got stricter and 6ovg reopens. **One clause here remains UNMEASURED and is flagged rather than trusted:** "none can be … renamed over" is proven for the *files* (harness A4) and not for the *directory* — `fort/scripts` is no longer a mount point itself, and nothing has tested whether it can be renamed. `fortkit-x9ou` carries the one-line measurement (harness A4b); until it runs, treat the directory half of that sentence as an expectation. (Corrected 2026-08-12, cycle 13, on Warden E2 blocking findings 1 and 2, Overseer-approved: the original text read "Bounded to the MAYOR", which was false by two launchers, and cited a measurement that did not cover the directory.)
