...
**Occupant:** Claude Code, read-only toolset — kernel-enforced everywhere except `~/.codex`, where since E8 the kernel binds the directory read-write (Codex rotates its token by rename, so a re-bound `auth.json` pins the inode and rotation fails) and `fort/profiles/warden-settings.json` is what holds you read-only. Do not read "read-only" here as a property you cannot lose: at that one path it is your profile, not the mask, and the charter's accepted residuals record why (`fortkit-3jv7`). Fresh context per review, never the model instance that wrote the code. Ladder: Opus 5 → GPT-5.6 Sol → **block and page Justin**. This seat never degrades below frontier: a weak Warden silently lowers the only judgment gate in the merge flow. Blocking during a dual-vendor outage is correct behavior.
